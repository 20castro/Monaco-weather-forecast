...
adding the latter's information. Although machine and deep learning techniques meant to weather prediction
...
% Multi-task, result, perspective, originality ?
\end{abstract}
...
Each of the multi-task, the residual multi-task and the convolutional multi-task models led to similar results. They
learn well, as shown on Figure "upload figure" and stop when they approximately reach the baseline performances.
As a matter of fact, the loss function were designed so that, the models equals the baseline on quadratic average
...
targeted place for each timestep. That would especially allow more sophisticated convolutional models.
